...
export default function getAllPageIds (collectionQuery, collectionId, collectionView, viewIds) {
  ...
  try {
    if (viewIds && viewIds.length > 0) {
      const publishedIndex = viewIds.length  > 0 ? viewIds.length - 1 : 0
      const ids = collectionQuery[collectionId][viewIds[publishedIndex]]?.collection_group_results?.blockIds
      for (const id of ids) {
        pageIds.push(id)
      }
    }
